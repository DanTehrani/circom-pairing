...
BUILD_DIR=../../build/fp12multiply
CIRCUIT_NAME=fp12multiply
...
start=`date +%s`
circom "$CIRCUIT_NAME".circom --O1 --r1cs --wasm --sym --wat --output "$BUILD_DIR"
end=`date +%s`
...
start=`date +%s`
rapidsnark "$BUILD_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json
end=`date +%s`
echo "DONE ($((end-start))s)"
